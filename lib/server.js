...
export function buildServer() {
  ...
  server.registerTool(
    "stores_nearby",
    {
      ...
      inputSchema: {
        ...
        maxItems: num(1, 1000, 100).describe("최대 반환 건수"),
  ...
  server.registerTool(
    "store_industry_mix",
    {
      ...
      inputSchema: {
        ...
        maxItems: num(1, 1000, 1000).describe("집계에 사용할 최대 점포 수"),
      },
    },
  ...
  server.registerTool(
    "search_permits",
    {
      ...
      description:
        ...
        "지역은 도로명주소 부분일치(addrLike)로 좁히는 것이 가장 편하다(예: '영등포구'). " +
        "좌표는 원본이 EPSG:5174라 서버가 WGS84로 변환해 돌려준다.",
      inputSchema: {
        type: z.string().describe(`업종명. 하나 선택: ${permit.PERMIT_TYPE_NAMES.join(", ")}`),
        addrLike: z.string().optional().describe("도로명주소 부분일치 (예: 영등포구, 서울특별시 강남구)"),
        orgCode: z.string().optional().describe("개방자치단체코드 (예: 3170000)"),
    ...
    async (a) => {
      try {
        if (a.countOnly) {
          ...
          return ok({ 업종: a.type, 조건: { ...a, countOnly: undefined }, 총건수: r.totalCount });
        }
    ...
    async ({ type, addrLike, fromYear, toYear }) => {
      try {
        ...
        const [all, active, closed] = await Promise.all([
          permit.fetchPermits({ type, addrLike, numOfRows: 1, pageNo: 1 }),
          permit.fetchPermits({ type, addrLike, salesStatus: "01", numOfRows: 1, pageNo: 1 }),
          permit.fetchPermits({ type, addrLike, salesStatus: "03", numOfRows: 1, pageNo: 1 }),
        ]);
  ...
  server.registerTool(
    ...
    async ({ lon, lat, radius, adm_cd, addrLike, permitType }) => {
      ...
      if (permitType && addrLike) {
        try {
          const [all, active, closed] = await Promise.all([
            permit.fetchPermits({ type: permitType, addrLike, numOfRows: 1, pageNo: 1 }),
            permit.fetchPermits({ type: permitType, addrLike, salesStatus: "01", numOfRows: 1, pageNo: 1 }),
            permit.fetchPermits({ type: permitType, addrLike, salesStatus: "03", numOfRows: 1, pageNo: 1 }),
          ]);
          const y = new Date().getFullYear();
          const recent = await permit.fetchPermits({
            type: permitType, addrLike, licenseFrom: `${y}-01-01`, numOfRows: 1, pageNo: 1,
          });
          out.인허가 = {
            업종: permitType,
            지역: addrLike,
