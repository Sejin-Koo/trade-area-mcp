 ...
 */
export async function callLivingPop({ ymd, tt, dong, start = 1, end = 100 } = {}) {
  const s = Math.max(1, Number(start) || 1);
  let e = Number(end) || s + 99;
  if (e - s + 1 > LIVINGPOP_MAX_ROWS) e = s + LIVINGPOP_MAX_ROWS - 1;

  // 뒤에 값이 있으면 앞자리를 %20 으로 채워 자리를 지킨다
  const parts = [ymd, tt === undefined || tt === null || tt === "" ? "" : String(tt).padStart(2, "0"), dong];
